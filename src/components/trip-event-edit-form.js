...
export const tripEventEditForm = ({type, destination, timeStart, timeEnd, price, offers}) => `<li class="trip-events__item">
...
<legend class="visually-hidden">
  ...
  <div class="event__field-group  event__field-group--destination">
    <label class="event__label  event__type-output" for="event-destination-1">
      ${type.title[0].toUpperCase()}${type.title.slice(1)} ${type.type === `activity` ? `in` : `to`}
    </label>
  ...
  <div class="event__field-group  event__field-group--time">
    ...
    <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${new Date(timeStart).toLocaleDateString()}">
    —
    <label class="visually-hidden" for="event-end-time-1">
      To
    </label>
    <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${new Date(timeEnd).toLocaleDateString()}">
  </div>
...
<section class="event__details">
  ...
  <section class="event__section  event__section--offers">
    ...
    type="checkbox" name="event-offer-${name}" ${offers.find((off)=> off.name === name) ? `checked=""` : ``}>
